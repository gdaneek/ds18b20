...
// version 1.0
#include "ds18b20.h"
inline void DS18B20::_ddr(uint8_t dir){if(dir)DDRB|=(1<<pin);else DDRB&=~(1<<pin);_delay_us(2);}  // Необходимо для правильного изменения состояния пина. Иначе не работает.
bool DS18B20::rst(void) 
{
	_ddr(1); 
  _delay_us(250);
  PORTB &= ~(1 << pin);  
...
void  DS18B20::sendBit(bool transmittedBit)
{
 	_ddr(1); 
  PORTB &= ~(1 << pin);
...
void DS18B20::sendByte(uint8_t transmittedByte) 
{
